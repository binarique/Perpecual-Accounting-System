...
$DESTINATION_FOLDER = $config->PROFILE_PHOTO_UPLOAD_FOLDER;
...
$responseMessage = array(
"message"=>"Unable to upload image, your image does not meet the requirements of 400x400 pixels and below, your image is ".$width."x".$height." pixels",
"status"=>"failed"
);      
...
if($uploader->cutPaste($TEMP_FOLDER.$final_file_name, $DESTINATION_FOLDER)){
    ...
    $responseMessage = array(
        ...
        "image"=>$helper->getHostURL().$config->PROFILE_PHOTO_FOLDER.$final_file_name,
        "message"=>"",
        "type"=>"product",
        "status"=>"success"); 
